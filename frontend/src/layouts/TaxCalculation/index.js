...
function TaxInfoDisplay() {
            ...
            <Table>
              <TableBody>
                <TableRow style={{ backgroundColor: "#e0f2f1" }}>
                  <TableCell variant="head" align="left"><b>Tax Rate</b></TableCell>
                  <TableCell variant="head" align="left"><b>Taxable Income Threshold</b></TableCell>
                </TableRow>
  ...
  function TaxCalculator() {
    ...
    const residentBrackets = [
      ...
      { rate: 0.10, threshold: 750000 },
      { rate: 0.15, threshold: 1150000 },
      { rate: 0.20, threshold: 1650000 },
      { rate: 0.25, threshold: Infinity }
    ];
  
    // Flat tax rate for non-residents
    const nonResidentRate = 0.30;
    ...
    const calculateResidentTax = (income) => {
      ...
      for (let i = 0; i < residentBrackets.length; i++) {
        ...
        if (remainingIncome > previousThreshold) {
          const taxableAmount = Math.min(remainingIncome - previousThreshold, threshold - previousThreshold);
                ...
                <form>
                  ...
                  {taxDetails && (
                    <MDBox mt={3}>
                      ...
                      <MDTypography variant="body1">
                        <strong>Residency Status:</strong> {taxDetails.residency === "resident" ? "Resident" : "Non-Resident"}
                      </MDTypography>
                      <MDTypography variant="body1">
